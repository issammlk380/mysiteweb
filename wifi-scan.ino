...
void setAlertMode(uint8_t ledPin, AlertType alertType) {
  Serial.print(F("[DEBUG] setAlertMode() START - Turning OFF all LEDs..."));
  allLedsOff();
  
  // ✅ FORCE: Turn off BLUE LED again (Wokwi bug workaround)
  digitalWrite(LED_BLUE, LOW);
  delay(10);
  
  Serial.println(F(" DONE"));
  
  Serial.print(F("[DEBUG] Turning ON LED pin "));
  ...
  // DEBUG: Vérifier état de toutes les LEDs
  Serial.print(F("[DEBUG] LED States after setAlertMode: RED="));
  Serial.print(digitalRead(LED_RED));
  Serial.print(F(" BLUE="));
  Serial.print(digitalRead(LED_BLUE));
  Serial.print(F(" YEL="));
  Serial.print(digitalRead(LED_YELLOW));
  Serial.print(F(" ORG="));
